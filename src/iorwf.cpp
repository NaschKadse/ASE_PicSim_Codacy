...
void iorwf::execute(const decode::decodedCmd &ldecoded) {
    if (ldecoded.dBit == 0) {
        ...
        picDatalocal->setCycle(ramlocal->setRam(ldecoded.filepos, picDatalocal->getWreg().to_ulong() | ramlocal->getRam(ldecoded.filepos).to_ulong(), picDatalocal->getCycle()));
